...
import
{ selectAll as getAllLembaga, getLembagaFromNIM}
  from '../models/lembaga';
 ...
 * @async
 */
export const getLembaga = async (nim?: string): Promise<string[]> => {
  // ga dikasih parameter
  const hasil: Array<string> = [];
  if (!nim) {
    (await getAllLembaga()).forEach(e => hasil.push(e.lembaga));
    return hasil;
  }

  try {
    (await getLembagaFromNIM(nim)).forEach(e => hasil.push(e.lembaga));
    return hasil;
  } catch (_) {
    console.error(_);
